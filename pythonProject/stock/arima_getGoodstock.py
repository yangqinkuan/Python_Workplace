...
def run_arma(original_ts, maxar=7, maxma=7):
    ...
    f = model.forecast(steps=1, alpha=0.05)[0]
...
if __name__ == '__main__':
    ...
    for i in range(0, 1):
        print(code.ix[0])
        data = pd.DataFrame(tusha.get_k_data('601998').loc[:, ["date", "close"]])
        ...
        # print(predict_ts.loc['2017-01-03', 'close'])
        count = 0
        # try:
        #     count = predict_ts.loc['2017-01-03', 'close']
        # except:
        #     count = 0
        # print(count.dtype)
        # if(count>0):
        ...
        for i in range(329, 573):
            data_in = original_ts.ix[i-300:i, :]
            predict, forecast = run_arma(data_in)
            rate = forecast[0]-original_ts.ix[i, 0]
        ...
        df.to_csv('C:/Users/Administrator/Desktop/stockpredict/ARIMA涨幅参数/601998.csv')
