...
if __name__ == '__main__':
    ...
    for i in range(0, data.shape[1]):
        ...
        plt.legend()
        plt.show()
    predict_list = np.array(predict.dropna())
    original_list = np.array(original_ts.ix[3:,0])
    print('aaa',len(original_list))
    print('bbb',len(predict_list))
    sum = 0
    for i in range(0,275):
        sum += abs(predict_list[i] - original_list[i]) / original_list[i]
    print('mean_error', sum/275)
