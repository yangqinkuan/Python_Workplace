...
forecast_price = {}
gains = {}
def method(start,end,code,forecast_price,gains):
    warnings.filterwarnings("ignore")

    for i in range(start, end):

        data = tusha.get_k_data(code.ix[i]).loc[:, ["date", "close"]]
...
if __name__ == '__main__':
    ...
    # forecast_price = multiprocessing.Manager().dict()
    # gains = multiprocessing.Manager().dict()
    # p1 = multiprocessing.Process(target=method, args=(0, 2, code, forecast_price, gains))
    # p2 = multiprocessing.Process(target=method, args=(2, 5, code,  forecast_price, gains))
    # p3 = multiprocessing.Process(target=method, args=(5, 7, code, forecast_price, gains))
    # p4 = multiprocessing.Process(target=method, args=(7, 10, code, forecast_price, gains))
    # p1.start()
    # p1.join()
    # p2.start()
    # p2.join()
    ...
    #     predict, forecast = run_arma(original_ts)
    #     print(forecast)
    #     forecast_price[data.columns[i]] = forecast
    #     print('预测新一天的值', forecast_price)
    #
