...
if __name__ == '__main__':
    ...
    codes = ['399390', '399387', '399381', '399385', '399382', '399383', ]
    for code in codes:
        forecast_price = {}
        gains = {}
        ...
        start_date = original_ts[(original_ts.date == '2017-01-03')].index.tolist()[0]
        original_ts = original_ts.set_index(["date", ])

        # print(len(original_ts))
        predict_ts = original_ts['2017-01-01':'2018-01-01']
        end_date = start_date+len(predict_ts)
        # print(len(predict_ts))
        print(start_date, end_date)
        print(original_ts.ix[start_date:end_date,:])
        # print(original_ts.ix[328-300:328, :])
        # 329-573
        ...
        for i in range(start_date, end_date):
            data_in = original_ts.ix[i-50+1:i+1, :]
            predict, forecast = run_arma(data_in)
            rate = forecast[2]/original_ts.ix[i, 0]-1
            predict_arr.append(rate)

        print(len(predict_arr))
        print(len(predict_ts))
        df = pd.DataFrame({'rate': predict_arr, },
                          index=predict_ts.index)
        df.to_csv('C:/Users/Administrator/Desktop/stockpredict/ARIMA1000指数涨幅/'+code+'.csv')
